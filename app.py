...
from sqlalchemy import create_engine, text, Integer, String, DateTime, Text
...
# YOUR LOCAL DATABASE
DATABASE_URL = "postgresql://postgres:peter2003$@localhost/drugai"
...
@login_required
def history():
    searches = Session.query(History)\
...
def search():
    ...
    try:
        with engine.connect() as conn:
            results = conn.execute(query, {"pattern": f"%{sickness}%"}).fetchall()
    ...
    # Save to history
    h = History(user_id=current_user.id, sickness=sickness, results=json.dumps(drugs))
    Session.add(h)
    Session.commit()
